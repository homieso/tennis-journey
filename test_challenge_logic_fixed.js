// 测试挑战计时逻辑（自然日制）- 修复时区问题
console.log('=== 测试挑战计时逻辑（自然日制） ===\n')

// 模拟解锁逻辑 - 使用UTC时间避免时区问题
function testUnlockLogic() {
  ...
  unlockDate.setUTCDate(unlockDate.getUTCDate() + 1) // 次日
  unlockDate.setUTCHours(0, 0, 0, 0) // 0:00:00.000 UTC
  
  const testTime1 = new Date('2026-02-28T00:00:01Z') // 次日0:00:01 UTC
  ...
  console.log(`  unlockDate: ${unlockDate.toISOString()}`)
  console.log(`  today1: ${today1.toISOString()}`)
  ...
  allyUnlockDate.setUTCDate(allyUnlockDate.getUTCDate() + 1)
  allyUnlockDate.setUTCHours(0, 0, 0, 0)
...
function testDeadlineLogic() {
  ...
  const start = new Date(startDate + 'T12:00:00Z') // 使用UTC
  
  // 第1天截止时间
  const day1Deadline = new Date(start)
  day1Deadline.setUTCHours(23, 59, 59, 999)
  console.log(`第1天截止时间: ${day1Deadline.toISOString()}`)
  
  // 第2天截止时间
  const day2Deadline = new Date(start)
  day2Deadline.setUTCDate(start.getUTCDate() + 1)
  day2Deadline.setUTCHours(23, 59, 59, 999)
...
function testMissedDayLogic() {
  ...
  dayDate.setUTCDate(start.getUTCDate() + 1) // 第2天
  
  const deadline = new Date(dayDate)
  deadline.setUTCHours(23, 59, 59, 999)
  
  const hasLog = logs.find(l => {
    const logDate = new Date(l.log_date + 'T12:00:00Z')
